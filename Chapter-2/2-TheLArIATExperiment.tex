...
\subsection{Collimators}\label{sec:Collimators}
...
We use the wire chamber system together with the bending magnets to measure the particles momentum.

In the simplest scenario, only one hit on each and every of the four wire chambers is recorder during a single readout of the detector systems.  Thus, we use the hits position in the two wire chambers upstream to the magnets to form a trajectory before the bent and the hits position in the two wire chambers downstream to the magnets to form a trajectory after the bent. We use the angles in the XZ plane between the upstream and downstream trajectories  to calculate the z component of the momentum as follows:
...
where $B_{eff}$ is the effective maximum field in a square field approximation,  $L_{eff}$ is the effective length of both magnets (twice the effective length of one magnet) and  3.3~$c^{-1}$ the conversion factor from from T*m to MeV/c. By using the hit positions on WC3 and WC4, we estimate the azimutal and polar angles of the particle trajectory, and we are able to calculate the other components of the momentum.

This simple scenario can get complicated in two ways:  the presence of multiple hits in a single wire chamber or by the absence of hits in one (or more) wire chamber. The first complication is due to beam pile up, while the latter is due to wire chamber inefficiency. Even in the case of multiple hits, at most one wire chamber track is reconstructed per event; since the magnets bend particles only in the X direction, we assume the particle trajectory to be roughly constant in the YZ plane, thus we keep the combination of hits most coplanar on YZ plane.  It is possible to reconstruct the particle momentum even in case of wire chamber inefficiency, if the information is missing in either of the two middle wire chambers (WC2 or WC3). %Assuming identical magnets, the upstream and downstream trajectories projected without bending should intersect at some point in the space between the magnets. Over a collection of many tracks, these points of intersection would trace a plane, therefore the term "midplane" is used. The completed half of the wire chamber track is projected to the midplane and the point of intersection is used with the incomplete half to finish the track. 

\textcolor{red}{BETTER EXPLAINATION OF HIGH YIELD}
\textcolor{red}{UNCERTAINTY ON MOMENTUM MEASUREMENT}
Events satisfying the simplest scenario of one single hit in each of the four wire chambers form the so called ``Picky track" sample.  We construct an other, higher statistics sample where we loose the requirements on single hit and wire chamber efficiency called "High Yield". For LArIAT Run II, the "High Yield" sample is 
\textcolor{red}{ HIGH YIELD increase in stat} the "Picky Tracks" sample. We use Picky tracks for calibration purposes: \textcolor{red}{ I DONT UNDERSTAND THIS Plotting the fractional error of the corrected three-point momentum compared to the known four-point momentum shows an uncertainty of approximately 2\%.}
...
\begin{figure}[htb]
\centering
\includegraphics[scale=0.35]{Chapter-2/Images//HVSchematic.png}\\
...
\begin{figure}[htb]
\centering
\includegraphics[scale=0.35]{Chapter-2/Images/DriftRegions.png}\\
...
\begin{figure}[htbp]
 \centering
 \includegraphics[width=1.0\textwidth]{Chapter-2/Images/LArIAT_FE_Electronics.png}
...
The LArIAT DAQ and trigger system governs the read out of all the many detectors forming LArIAT. 
The CAEN V1495 module featuring a user-programmable FPGA  is the core of this system.  Every 10~ns, this module checks for matches between sixteen logical inputs and user-defined patterns in the trigger menu; if it finds a match for two consecutive clock ticks, that trigger fires.


% converts signals from analog to digital, interfaces and synchronizes with the accelerator system, stores raw data on disk, transfers data to archival storage and conduct a first pass data quality monitoring. 




%A CAEN V1495 board is used to perform the trigger of the whole detectors system. It is a VME board, which can be directly customized by the user through two programmable FPGA. For LArIAT experiment 16 trigger input signals are used, see Fig. 2.14. Through the FPGA configuration file  V1495.config  it is possible to define different trigger conditions - trigger output paths, with several logic combinations of the input signals.
%The former trigger output path is the one that selects particles from the beam that cross the whole beamline and get to the TPC.
%Moreover it's possible to add a delay to the trigger and to set some combinations of signals as a VETO.
%Waveforms from beamline detectors and optical devices (PMTs and SiPMs) inside the cryostat are collected using two CAEN V1751 boards (For v1751 boards inputs, see Table 2.2.). They are both fed by the V1495 fast trigger. Each has a unit boardId and produces CAENFragment raw data objects.
%The CAEN V1751 Waveform Digitizer [40] is VME module housing 8 Channel 10 bit 1 GS/s Waveform Digitizer with 1 Vpp input dynamic range; the range that has been set for our purposes is 204 mV, so 1 ADC count corresponds to 0.2 mV. The input channel provides a programmable voltage o set in the ± 0.5 V range too; we used this feature to adjust baseline o sets on our signals. The time window can also be adjusted by user maintaining the same sample %frequency (1 GS/s); we've started with a 7.168 ?s
%time window, but after preliminary studies on the signals collected by PMTs and SiPMs we've moved to a 14.336 ?s window that could allow us to sample over all the slow LAr scintillation component (?S) signal tails, to study single photoelectron response of the optical devices.


%The LArIAT data acquisition system is triggered to read out the digitizing buffers by a CAEN V1495 module. The V1495 is a powerful, easily configurable coincidence module featuring a user-programmable FPGA.  Sixteen logical inputs  are sampled on a 10~ns clock, checking for matches to any of the sixteen user-defined patterns in the trigger menu.  If a given trigger pattern is satisfied for two consecutive clock ticks, that pattern fires. 
...
\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{Chapter-2/Images/BeamOverview.png}
...
\begin{figure}[htb]
\centering
\includegraphics[scale=0.5]{Chapter-2/Images/BeamPosition.png}
...
\begin{figure}[htb]
\centering
\includegraphics[scale=0.6]{Chapter-2/Images/RunConditions.png}
