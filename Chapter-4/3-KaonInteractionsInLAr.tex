...
Assuming a beam of pure pions gets to the TPC, let us explicit some of the variables in the kinetic energy equation \ref{eq:KEj}  to point out the important quantities in the error budget,
...
Let us start by discussing the uncertainty on $E_{Beam}^{kin}$. Since we are assuming a beam of pions, the uncertainty on the value of mass of the pion ($m_{Beam}$) as given by the pdg is irrelevant compared to the momentum uncertainties, thus $\delta E_{Beam}^{kin} = \delta p_{Beam}^{kin}$. \textcolor{blue}{Ideas on how to estimate the momentum uncertainty. Base line:  4-points track. In case of 3-points track, add an additional 2\%. }


KEWC = (pWC2 - m2)?   => ?m very small, ?KEWC = ?pWC
ELOSS                        => ?ELOSS MC truth spread
dE/dxi                  => ?dE/dxi MC Reco - Data comparison
dxi                        => ?dxi      MC Reco - Data comparison
?TOT, ?i<j                  => Efficiency correction
