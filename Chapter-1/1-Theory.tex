...
A summary of the pion final states in order of pion multiplicity is given in table \ref{tab:pionFinal}. Pion capture on the argon orbital and pion decay at rest dominate the cross section under 100 MeV. The pion decay life time is $\tau_\pi = 2.6 \times 10^-8$ s and the main decay mode is  $\pi^-\rightarrow \mu^- + \bar\nu_\mu$ (BR 99.98\%); for pion decay at rest, the muon is emitted with an energy of $\sim5$ MeV and it decays into $\mu^-\rightarrow e^- + \bar\nu_e + \nu_\mu$ in $\tau_\mu = 2.3 $ $\mu$s . Since pion capture can be considered an electromagnetic process and pion decay is a weak process, this energy region is purposely excluded from the hadronic cross section measurement.
