...
%%%%%%%%%%
Hints to the existence of at least one additional neutrino, in the form of various anomalies, have been puzzling physicists almost from the beginning of neutrino oscillation searches. 
...
%%%%%%%%%%
The measurement of non-zero value for the oscillation parameter $\theta_{13}$ unleashes the exploration of low-energy CP violation in the neutrino sector. Using the parameterization of the PMNS matrix shown in Equation \ref{eq:PMNS},  the difference in the probability of $\nu_e$ to $\nu_\mu$ oscillation and the probability of $\bar\nu_e$ to $\bar\nu_\mu$ oscillation can be parametrized as follows,
\begin{equation}
P_{\nu_e\rightarrow\nu_\mu} - P_{\bar\nu_e\rightarrow\bar\nu_\mu} = J \cos\Big(\pm\delta - \frac{\Delta_{31} L}{2}\Big) \sin\Big(\frac{\Delta_{21} L}{2}\Big)\sin\Big(\frac{\Delta_{31} L}{2}\Big)
\end{equation}
\textcolor{red}{asymmetry formula}
where. 
  The magnitude of the CP- violating terms in the oscillation depends  directly on the size of the Jarlskog Invariant [56], but it aslo related to the octant and the the mass hiararchy. 


Exciting theoretical results tie low-energy CP violation in the neutrino sector directly to the generation of the baryon asymmetry of the Universe at the Grand Unified Theory scale \textcolor{red}{a couple of cit would be nice}. According to theoretical model described in \cite{PASCOLI20071}, for example, leptogenesis can be achieved if $|\sin\theta_{13} \sin \delta| > 0.11$, i.e. $\sin \delta > 0.7$.
