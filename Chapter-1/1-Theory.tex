...
In 1937, Majorana demonstrated that the introduction of a two components spinor is sufficient to describe a neutral massive fermion, under the assumption that the chiral components $\psi_R$ and $\psi_L$ are correlated as follows
$$\psi_R = \mathcal{C}\bar\psi_L$$ where $\mathcal{C}$ is the charge conjugation matrix. Neutrinos are the only neutral elementary particles in the SM, thus the only possible Majorana particle candidate.
\textcolor{red}{ a few words on see-saw}

From a kinematic point of view, Dirac and Majorana neutrinos satisfy the same energy-momentum dispersion relationship. Thus, it is impossible to discern the neutrino nature through kinematic effects such as neutrino oscillations.

Il modo più promettente per capire se il neutrino è una particella di Majorana è cercare sperimentalmente il doppio decadimento ? senza neutrini.
Il doppio decadimento. E? chiaro inoltre che tale processo viola il numero leptonico di due unità

The puzzle of the neutrino mass hints to the fascinating idea of considering the SM as an effective theory at low energy, obtained as the limit of a more complete theory.
